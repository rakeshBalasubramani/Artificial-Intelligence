...
def depthFirstSearch(problem):
    ...
    while not fringe.isEmpty():
        temp = fringe.pop()
  	currentState = temp[len(temp)-1]
  	currentState = currentState[0]
  	if problem.isGoalState(currentState):
            return [path[1] for path in temp][1:]
  	if currentState not in visited:
  	    visited.append(currentState)
            for successor in problem.getSuccessors(currentState):
                if successor[0] not in visited:
                    successorTemp = temp[:]
  		    successorTemp.append(successor)
  		    fringe.push(successorTemp)
    return []
    util.raiseNotDefined()
  	    ...
  	    visited.append(current)
            for successor in problem.getSuccessors(current):
                if successor[0] not in visited:
                    ...
                    successor[2]=cost+successor[2]
                    successor=tuple(successor)
                    successorPath = path[:]
  		    successorPath.append(successor)
  		    fringe.push(successorPath,successor[2])
  	    ...
  	    visited.append(current)
            for successor in problem.getSuccessors(current):
                if successor[0] not in visited:
                    ...
                    successorPath = path[:]
  		    successorPath.append(successor)
  		    fringe.push(successorPath,successor[2]+heuristic(successor[0],problem))
