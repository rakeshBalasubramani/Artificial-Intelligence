...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        print "1",self.getJailPosition()
        print "2",noisyDistance
        print "3",emissionModel
        print "4",pacmanPosition
        print "5",self.beliefs[]
        print "6",self.legalPositions
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, p in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + p*self.beliefs[oldPos]
                #print allPossible
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        no_of_Particles=0
        self.particles =[]
        while no_of_Particles < self.numParticles:
            for pos in self.legalPositions:
                if no_of_Particles < self.numParticles:
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: 
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] += emissionModel[distance] * oldBelief[pos]
            if not any(allPossible.values()):
                self.initializeUniformly(gameState)
            else:
                t = []
                for i in range(0, self.numParticles):
                    t.append(util.sample(allPossible))
                    #print temp
                self.particles = t
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        no_OfParticles=0
        self.particles = []
        while no_OfParticles < self.numParticles:
            for position in possiblePos:
                if no_OfParticles < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        for index, p in enumerate(self.particles):
            ...
            allPossible[p] += partial
        if not any(allPossible.values()):
            self.initializeParticles()
            for index, p in enumerate(self.particles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:
                        p = self.getParticleWithGhostInJail(particle, i)
        else:
            allPossible.normalize()
            x = []
            for i in range(0, self.numParticles):
                x.append(util.sample(allPossible))
            self.particles = x
