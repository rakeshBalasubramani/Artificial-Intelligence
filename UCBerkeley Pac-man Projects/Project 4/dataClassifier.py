...
def enhancedFeatureExtractorDigit(datum):
    ...
    for y in h:
        ...
        feat2 = 0
        for x in w:
            on = datum.getPixel(y,x)>0
            off = datum.getPixel(y,x)==0
            con1 = feat1 == 0 and on
            aCon = feat1 == 1 and off
            fea1 = pix == 0 and aCon
            con2 = feat1 == 1 and pix == 1 and on
            fea2 = pix == 1 and feat2 == 1 and aCon
            if con1:
                feat1 = 1
            if fea1:
                pix = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    gPos = suc.getGhostPositions()
    pPos = suc.getPacmanPosition()
    foods = suc.getFood()
    fPos = []
    for x in range(foods.width):
        for y in range(foods.height):
            if foods[x][y]==True:
                fPos.append((x, y))
    gDis = [util.manhattanDistance(g, pPos) for g in gPos]
    fDis = [util.manhattanDistance(g, pPos) for g in fPos]
    fDis.sort()
    for i in range(len(fDis)):
        features['foodDistance_{0}'.format(i)] = 1.0/(fDis[i]+1)
    features['minGhostDistance'] = min(gDis)
    return features
